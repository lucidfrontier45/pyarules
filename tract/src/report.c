...
static int isr_tidout (ISREPORT *rep, TID tid)
...
static int isr_occout (ISREPORT *rep, ITEM occ)
{                               /* --- print a positive integer */
  int  i = BS_INT;              /* loop variable */
  char buf[BS_INT];             /* output buffer */
...
ISREPORT* isr_create (ITEMBASE *base)
{                               /* --- create an item set reporter */
  ...
  size_t     sum;               /* sum of item name lengths */
  #endif
  ...
  rep->sum     = 0;             /* clear the total name length */
  ...
  rep->pxpp    = (ITEM*)  malloc((size_t)(4*n+3) *sizeof(ITEM));
  rep->supps   = (RSUPP*) malloc((size_t)(n+1)   *sizeof(RSUPP));
  rep->wgts    = (double*)calloc((size_t)(n+n+1), sizeof(double));
  rep->stats   = (size_t*)calloc((size_t)(n+1),   sizeof(size_t));
  if (!rep->pxpp || !rep->supps || !rep->wgts || !rep->stats) {
    isr_delete(rep, 0); return NULL; }
  memset(rep->pxpp, 0, (size_t)(n+1) *sizeof(ITEM));
  rep->pexs    = rep->pxpp +n+1;/* allocate memory for the arrays */
  rep->items   = rep->pexs += n;/* and organize and init. the arrays */
  rep->iset    = rep->items+n+1;
  rep->ldps    = rep->wgts +n+1;
  *rep->wgts   = (double)(*rep->supps = ib_getwgt(base));
  ...
  for (sum = 0, i = 0; i < n; i++) {
    ...
    if (!rep->scan)             /* if to use the items names directly */
  ...
  rep->sum = sum;               /* note total length of item names */
  #endif
...
{                               /* --- delete an item set reporter */
  ...
  if (rep->buf) free(rep->buf); /* delete the file write buffer */
...
{                               /* --- fast output of an item set */
  ...
  isr_putsn(rep, rep->foinfo, rep->size);      /* and its support */
}  /* fastout() */
...
{                               /* --- report the current item set */
  ...
  if (rep->fast)                /* format support for fast output */
    rep->size = sprintf(rep->foinfo, " (%"RSUPP_FMT")\n",
                        rep->supps[rep->cnt]);
...
{                               /* --- report a sequence rule */
  ...
  if (rep->rulefn) {            /* if a reporting function is given */
    rep->eval = eval;           /* note the evaluation */
    rep->rulefn(rep, rep->ruledat, items[n-1], body, head);
  }                             /* call the reporting function */
  if (!rep->file) return 0;     /* check for an output file */
  i = rep->cnt; rep->cnt = n;   /* note the number of items */
  isr_puts(rep, rep->hdr);      /* print the record header */
