...
#define VERSION     "version 2.5 (2014.10.24)         " \
...
{                               /* --- add a transaction */
  ...
  for (tract = 0; --n >= 0; items++) {
    if      (*items <  0) tract |= (BITTA)*items;
    else if (*items < 16) tract |= (BITTA)(1 << *items);
...
{                               /* --- filter items from projection */
  ...
  for (mask = 0; --n >= 0; ) {  /* traverse the items */
    ...
    else mask |= (BITTA)(1 << n);  /* if the item is simply frequent, */
  }                                /* set its bit in the item mask */
...
{                               /* --- main function */
  ...
  if (eval == 'b')              /* set the evaluation function */
    isr_seteval(report, isr_logrto, NULL, +1, 0.01*thresh);
