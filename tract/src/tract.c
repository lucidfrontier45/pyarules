...
#define VERSION     "version 2.3 (2014.10.24)         " \
...
{                               /* --- create an item base */
  ...
  if (mode & IB_OBJNAMES) {     /* if general objects as names, */
    ...
    base->idmap = idm_create(0, 0, hashfn, cmpfn, data, delfn); }
  else                          /* if item names are strings */
    base->idmap = idm_create(0, 0, ST_STRFN, (OBJFN*)0);
...
{                               /* --- finalize transaction buffer */
  ...
  for (i = 0; i < t->size; i++) {
    ...
    itd->frq += t->wgt;         /* traverse the items and */
    itd->xfq += wgt;            /* sum the transaction weights */
...
{                               /* --- read a transaction */
  ...
  if (base->mode & IB_WEIGHTS){ /* if the items carry weights */
    ...
    for (i = 0; i < x->size; i++) {
      ...
      itd->frq += x->wgt;       /* traverse the items and */
      itd->xfq += w;            /* sum the transaction weights */
    } }                         /* and the transaction sizes */
  else {                        /* if the items do not carry weights */
    t = (TRACT*)base->tract;    /* get the transaction buffer and */
    if (mode & TA_TERM)         /* if requested, store term. item */
